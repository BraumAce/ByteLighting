---
url: 'https://blog.bytelighting.cn/program/tools/vpn-proxy-setup.md'
---
# 科学上网与本地代理
...
1. 准备一台 **海外服务器**，假设名为 `服务器A`（IP 为 `123.123.123.123`），并设置安全组开放 `10123` 端口（开放端口很重要，同时也很危险，详见 [放行端口的步骤](./vpn-proxy-setup.md#放行端口的步骤)）。
...
6. 打开 [代理客户端](https://github.com/v2rayU/v2rayU) 的设置界面，将 `123.123.123.123.10123` 代理服务修改为 `456.456.456.456.20456`。然后测试代理服务是否可用。示例配置在 [本地代理客户端](./vpn-proxy-setup.md#本地代理客户端)
