---
url: 'https://blog.bytelighting.cn/domains/algorithms/templates/basic-algorithms.md'
---
# 基础算法

涵盖快速排序、归并排序、二分、高精度、前缀和与差分、位运算、双指针、离散化、区间合并等基础算法模板。
...
② `upper_bound(begin, end, val)` 可以在区间 `[begin, end)` 中找到 `val` 最后一次出现的位置的\_\_后面一位\_\_ 。
...
如果想将一个单元清零，使其全部二进制位为 0，只要\_\_与一个各位都为零的数值相与\_\_，结果为零。
...
![bit\_operation](https://cloud.bytelighting.cn/f/5gF5/bit_operation.png)
...
作用：**返回 x 的最后一位 1**，比如 `x = 1010, 则 lowbit(x) = 10` ，`x = 101000, 则 lowbit(x) = 1000` .
