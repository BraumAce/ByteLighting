---
url: 'https://blog.bytelighting.cn/domains/algorithms/templates/math.md'
---
# 数学知识

涵盖质数、约数、欧拉函数、快速幂、扩展欧几里得、中国剩余定理、高斯消元、组合数、容斥原理、博弈论等数学算法模板。
...
> $ϕ(N) = N × \frac{p\_1−1}{p\_1} × \frac{p\_2−1}{p\_2} × … × \frac{p\_m−1}{p\_m}$
...
2. 将 $a^b$ 用 $a^{2^0}, a^{2^1}, a^{2^2}, ..., a^{2^{logk}}$ 这 k 个数来组合，即组合成 $a^b = a^{2^{x\_1}} × a^{2^{x\_2}} × ... × a^{2^{x\_t}} = a^{2^{x\_1} + 2^{x\_2} + ... + 2^{x\_t}}$​
...
给定 $2n$ 个整数 $a\_1, a\_2, ... , a\_n$ 和 $m\_1, m\_2, ..., m\_n$ ，求一个最小的非负整数 $x$ ，满足 $∀i ∈ \[1, n], x ≡ m\_i(mod \ a\_i)$ .
...
给定两个正整数 a 与 b ，求 $C\_a^b \ mod(1e9 + 7)$

**递推式：**

$C\_a ^ b = C\_{a - 1} ^ {b - 1} + C\_{a - 1} ^ {b}$
...
$C\_a^b = \frac{a!}{b!\ \* \ (a - b)!} = a! \ \* \ infact(b!) \ \* \ infact((a - b)!)$
...
给定 n 组询问，每组询问给定三个整数 a,b,p，其中 p 是质数，请你输出 $C\_a^b \ mod \ p$ 的值。

1. $Lucas$ 定理：$C\_a^b ≡ C\_{a % p} ^{b % p} \ \* \ C\_{\frac{a}{p}} ^{\frac{b}{p}} \ (mod\ p)$
2. $C\_b^a = \frac{a!}{(a−b!)∗b!} = \frac{a∗(a−1)∗(a−2)∗…∗(a−b+1)∗(a−b)∗…∗1}{(a−b)∗(a−b−1)∗…∗1∗b!} = \frac{a∗(a−1)∗(a−2)∗…(a−b+1)}{b!}$
...
请你求出 1 ~ n 中能被 $p\_1, p\_2, ..., p\_m$​ 中至少一个数整除的整数有多少个。

记 $S\_i$ 为 1 ~ n 中能整除 $p\_i$​ 的集合，根据容斥原理，所有数的个数为各个集合的并集，计算公式如下
...
1. 必胜状态，先手进行\_\_某一个操作\_\_，留给后手是一个必败状态时，对于先手来说是一个必胜状态。即\_\_先手可以走到某一个必败状态\_\_。
2. 必败状态，先手\_\_无论如何操作\_\_，留给后手都是一个必胜状态时，对于先手来说是一个必败状态。即\_\_先手走不到任何一个必败状态\_\_。
