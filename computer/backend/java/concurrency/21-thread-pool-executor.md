---
url: >-
  https://blog.bytelighting.cn/computer/backend/java/concurrency/21-thread-pool-executor.md
---
# 线程池 ThreadPoolExecutor 实现原理

本文介绍 ThreadPoolExecutor 的核心构造参数、执行流程（execute 方法源码分析）、关闭策略以及线程池参数的合理配置方案。
...
5. **workQueue**：阻塞队列。
   ...
   用于保存任务的阻塞队列。可以使用 ArrayBlockingQueue, LinkedBlockingQueue, SynchronousQueue, PriorityBlockingQueue。
