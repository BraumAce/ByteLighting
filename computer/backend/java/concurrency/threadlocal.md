---
url: 'https://blog.bytelighting.cn/computer/backend/java/concurrency/threadlocal.md'
---
# 并发容器之 ThreadLocal

本文深入分析 ThreadLocal 的实现原理，包括 set/get/remove 核心方法、ThreadLocalMap 的散列表设计（开放寻址法）、Entry 弱引用机制以及潜在的内存泄漏问题。
...
![ThreadLocal各引用间的关系](https://cloud.bytelighting.cn/f/RzaCg/17.1%20ThreadLocal%E5%90%84%E5%BC%95%E7%94%A8%E9%97%B4%E7%9A%84%E5%85%B3%E7%B3%BB.png)
...
![理想散列表的示意图](https://cloud.bytelighting.cn/f/jXmsO/17.2%20%E7%90%86%E6%83%B3%E6%95%A3%E5%88%97%E8%A1%A8%E7%9A%84%E7%A4%BA%E6%84%8F%E5%9B%BE.png)
...
![分离链表法示意图](https://cloud.bytelighting.cn/f/Kgws9/17.3%20%E5%88%86%E7%A6%BB%E9%93%BE%E8%A1%A8%E6%B3%95%E7%A4%BA%E6%84%8F%E5%9B%BE.png)
...
![开放寻址法示意图](https://cloud.bytelighting.cn/f/8djT3/17.4%20%E5%BC%80%E6%94%BE%E5%AE%9A%E5%9D%80%E6%B3%95%E7%A4%BA%E6%84%8F%E5%9B%BE.png)
...
`0x61c88647` 这个数是有特殊意义的，它能够保证 hash 表的每个散列桶能够均匀的分布，这是 `Fibonacci Hashing`。也正是能够均匀分布，所以 threadLocal 选择使用开放寻址法来解决 hash 冲突的问题。
...
自然而然，$Josh \ Bloch \ and \ Doug \ Lea$ 大师考虑到了这种情况，在 `set` 方法的 for 循环中寻找和当前 Key 相同的可覆盖 entry 的过程中通过 `replaceStaleEntry` 方法解决脏 entry 的问题。如果当前 `table[i] = null` 的话，直接插入新 entry 后也会通过 `cleanSomeSlots` 来解决脏 entry 的问题。
...
根据源码可知，在第一次为 threadLocal 进行赋值的时候会创建初始大小为 $16$ 的 threadLocalMap，并且通过 `setThreshold` 方法设置 threshold，其值为 `当前哈希数组长度` \* $\frac{2}{3}$，也就是说加载因子为 $\frac{2}{3}$。
...
方法逻辑请看注释，新建一个大小为原来数组长度的两倍的数组，然后遍历旧数组中的 entry 并将其插入到新的 hash 数组中。需要注意的是，在扩容的过程中针对脏 entry 的话会令 value 为 `null`，以便能够被垃圾回收器回收，解决隐藏的内存泄漏的问题。
