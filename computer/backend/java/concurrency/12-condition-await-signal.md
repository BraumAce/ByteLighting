---
url: >-
  https://blog.bytelighting.cn/computer/backend/java/concurrency/12-condition-await-signal.md
---
...
我们知道在锁机制的实现上，AQS 内部维护了一个同步队列，如果是独占式锁的话，所有获取锁失败的线程的尾插入到**同步队列**，同样的，Condition 内部也是使用同样的方式，内部维护了一个**等待队列**，所有调用 `condition.await` 方法的线程会加入到等待队列中，并且线程状态转换为等待状态。另外注意到 ConditionObject 中有两个成员变量：
...
![](https://cloud.bytelighting.cn/f/kPAh5/12.1%20debug%E6%A8%A1%E5%BC%8F%E4%B8%8B%E6%83%85%E6%99%AF%E5%9B%BE.png)
...
![](https://cloud.bytelighting.cn/f/2oZtZ/12.2%20%E7%AD%89%E5%BE%85%E9%98%9F%E5%88%97%E7%9A%84%E7%A4%BA%E6%84%8F%E5%9B%BE.png)
...
![AQS 持有多个 Condition](https://cloud.bytelighting.cn/f/D9DiX/12.3%20AQS%E6%8C%81%E6%9C%89%E5%A4%9A%E4%B8%AACondition.png)
...
具体请看注释，在第 $1$ 步中调用 `addConditionWaiter()` 将当前线程添加到等待队列中，该方法源码为：
...
到目前为止，这两段代码已经解决了前面的两个问题的答案了，还剩下第三个问题 —— 怎样从 `await` 方法退出？
...
![](https://cloud.bytelighting.cn/f/Lgdhl/12.4%20await%E6%96%B9%E6%B3%95%E7%A4%BA%E6%84%8F%E5%9B%BE.png)
...
**调用 condition 的 `signal/signalAll` 方法可以将等待队列中等待时间最长的节点移动到同步队列中**，使得该节点能够有机会获得 lock。按照等待队列是先进先出（FIFO）的，所以等待队列的头节点必然会是等待时间最长的节点，也就是每次调用 condition 的 `signal` 方法是将头节点移动到同步队列中。
...
![](https://cloud.bytelighting.cn/f/Wzyh7/12.5%20signal%E6%89%A7%E8%A1%8C%E7%A4%BA%E6%84%8F%E5%9B%BE.png)
...
![Condition 下的等待通知机制](https://cloud.bytelighting.cn/f/lyDSD/12.6%20condition%E4%B8%8B%E7%9A%84%E7%AD%89%E5%BE%85%E9%80%9A%E7%9F%A5%E6%9C%BA%E5%88%B6.png)
