...
description: ByteLighting 博客（VuePress 2 中文技术博客）的写作与维护规范集合。命中以下任一场景时必须使用：(1) 创建 git commit / push / PR；(2) 编辑 src/program/reading/ 月度阅读记录；(3) 新增、移动、归类 src/ai/ 下任何文件或子目录；(4) 填写或修改文章 frontmatter 的 category / tag 字段。规范源文件位于 docs/conventions/{git-commit,reading-format,ai-structure,categorization}.md。
---
...
**核心**：frontmatter `order: -{month}` + `category: 文章`；正文以 `<!-- more -->` 分隔摘要与正文；条目格式严格，描述要给出方法论 / 关键路径，不写「值得一读」之类无信息量修饰；微信公众号链接抓取走 `mcp__plugin_everything-claude-code_exa__web_fetch_exa`。
...
- **凭印象 = 失误**：上述 4 类场景下，**必须**实际 Read 对应规范文档，哪怕你"觉得记得"
- **跨场景叠加**：一次操作可能命中多个场景（例：「写一篇 AI 文章并提交」→ 命中 3 + 4 + 1），命中几个就读几个
